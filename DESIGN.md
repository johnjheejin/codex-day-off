...
- After collection ends, the completed flower group makes one restrained, 2.8-second turn to reveal its depth, then returns to the front. Keep the sky unobscured. A `Keep this sky` control or Escape skips the reveal. Reduced motion goes straight to the live result. Hidden-page time does not consume the reveal. Do not add flowers or rearrange the composition. The reveal draws at no more than 60fps, then rests.
...
- The completed sky is the default result surface and stays live. Keep the entire canvas visible with no large title or duplicate artwork preview. A small perimeter colophon reports the real thought/color totals and thirty-second duration; do not infer mood, pace, or achievement.
- Use action names `Touch` and `Turn`, without 2D/3D terminology. Touch returns the group to its front view and brushes nearby flowers; Turn rotates it through dragging or arrow keys. Reset returns the view to its original orientation. A separate view pose preserves the source composition and PNG.
- Preserve a brief touch through its first response frame even when rendering is delayed. Clear pending input when leaving, suspending, or switching the view; settling still ends without an idle rendering loop.
- The Share button opens a modal with the original uncropped PNG, the existing image/download actions and social destinations. Closing the modal returns to the same live sky and camera angle. Pause rendering beneath the modal and keep its close control reachable while scrolling.
- The Afterglow wordmark returns to the landing. During play, pause time/audio/rendering and ask whether to end the current pause; cancelling resumes from the same point. From a completed sky, return directly. Keep the most recent completed sky and original PNG in memory for this page, including while a new pause is in progress. The landing offers `Return to your sky`. Restore the composition and its chosen scene after viewport or landing-scene changes; do not imply persistence after a reload or a personal share link.
...
- The live result uses a compact bottom dock for the colophon, Touch/Turn, Reset and Share. All controls fit a 320px phone and meet the 44px target. Keep the persistent top bar clear. In the sharing modal, an uncropped thumbnail sits beside the compact title on mobile, followed by all six destinations in the scroll flow.
- Sharing and confirmation dialogs remain vertically scrollable when their content exceeds the available screen. The live sky itself does not scroll.
