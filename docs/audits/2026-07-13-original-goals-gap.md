...
| 항목 | 판단 | 다음 결정 |
| --- | --- | --- |
| 기기별 성능 안전성 | 부분 구현이었으나 사고로 불충분함이 확인됨 | ADR-0002 보호장치 적용 및 실기기 확인 |
| 밝은 플레이 화면 | 기존에는 없음 | `Light field / Night field`로 구현 |
| 관찰 가능성과 평가 기준 | `?debug=1`과 수동 리뷰는 있으나 자동 회귀 기준 없음 | 픽셀 예산·목표 FPS·콘솔 오류를 검증 체크리스트에 고정 |
| 정적 HTML 제출 번들 | 루트 ZIP이 6월 18일 구버전 | 최신 커밋으로 재생성하거나 저장소에서 제외 |
| GitHub 프로젝트 설명 | About이 여전히 60초로 표시 | 30초 설명으로 변경 |
...
1. P0 모바일 안전 패치와 밝은 필드 배포
2. Chrome Beta·Samsung Internet 실기기 확인
3. 구버전 ZIP과 GitHub About 정리
