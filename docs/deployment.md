...
2026-09-06 **새 아이덴티티와 README까지 GitHub 자동 배포로 공개했다.** 사용자가 발급을 승인한 Pages Edit 토큰을 GitHub 저장소 비밀값에 연결했다. 개인 Wrangler 로그인 없이 GitHub Actions가 기존 프로젝트에 공개 배포를 수행한다.

- [성공한 push 실행](https://github.com/johnjheejin/codex-day-off/actions/runs/34003658777), source `8da61a4`. 빌드·번들·브랜드 자산 일치·단위 21개·브라우저 32개(총 53개)를 통과한 뒤 게시했다.
- Cloudflare production: `d6a52ab5-5740-46a3-8439-8d5c015b2cc5`, [고정 배포 주소](https://d6a52ab5.codex-day-off.pages.dev). 고정 주소의 15개 파일(803,418바이트)과 브라우저 32개를 GitHub에서 검증했다. 기존 8개 런타임 파일과 브랜드 SVG·아이콘·링크 미리보기 7개다.
- Pages API에서 공개 도메인이 활성 상태이고, 프로젝트의 실제 production 배포가 이번 커밋과 고정 URL에 일치함을 확인했다.
- **GitHub에서 공개 도메인의 전체 콘텐츠 검증은 보안 challenge로 미완료다.** HTML과 서체 README 요청은 HTTP 403 / `cf-mitigated: challenge`, 나머지 13개 파일은 일치했다. 이 상태는 경고와 `matches: false` 기록으로 남기며, 공개 도메인 브라우저 검사 성공으로 취급하지 않는다.
- 같은 공개 도메인을 이 컴퓨터에서 검사하면 15개 파일이 모두 일치하고 Chrome Beta 브라우저 32개도 통과한다. 보안 설정을 바꾸거나 challenge를 우회하지 않았다.
- [CI 배포·도메인 연결·환경별 파일 대조·공개 브라우저 기록](../assets/verification/2026-09-06-identity/production.json). Actions의 `verification`과 `production-verification` 아티팩트는 14일 보관한다. [이전 라이브 결과 배포 기록](../assets/verification/2026-09-06-kept-sky/production.json)은 이력으로 보존한다.
...
2. Node 24에서 `npm ci`, 재빌드 후 체크인된 번들·브랜드 SVG·인라인 헤더와 일치 확인, 정책·단위·브라우저 검사를 실행한다. Linux에서는 소프트웨어 WebGL을 사용하므로 물리 GPU 성능을 증명하지 않는다.
3. 검사 성공 뒤 `scripts/prepare-pages.mjs`가 필요한 15개 런타임·브랜드 파일만 `dist/`에 모은다. 테스트 결과, 인증 파일, 옛 ZIP과 개발 의존성은 업로드하지 않는다.
4. Wrangler 4.129.0이 동일 Pages 프로젝트의 `main`으로 게시하고 Git 커밋을 배포에 기록한다.
5. 고정 배포 URL의 15개 파일을 SHA-256으로 대조하고 브라우저 32개를 실행한다. 이후 Pages API의 `canonical_deployment`와 활성 custom domain을 조회해 배포 URL·전체 Git SHA·공개 도메인의 연결을 확인한다.
